           ...
           <form class="card" action="/collect/aggregated-model/formular/save" method="post" ng-controller="AppController as vm">
		       <div class="card-header">
		       		<xsl:if test="item and item/id">
			            <h3 class="card-title"><xsl:text>Modifier formule </xsl:text><i><xsl:value-of select="item/name"/></i></h3>
			        </xsl:if>
			        ...
			        <div class="card-options">
	                   <xsl:if test="item and item/id">
	                    	<a ng-if="vm.item.condition.state == 'removed'" href="javascript:void(0)" role="button" class="btn btn-primary" ng-click="vm.addFormularCondition()">
			                  	<i class="fa fa-plus"></i> Ajouter condition
			                </a>
			                <a ng-if="vm.item.condition.state == 'added'" href="javascript:void(0)" role="button" class="btn btn-primary" ng-click="vm.removeFormularCondition()">
			                  	<i class="fa fa-minus"></i> Supprimer condition
			                </a>
	                    </xsl:if>
	                </div>
		       </div>
               <div class="card-body">  
				                      ...
				                      <tbody>
				                          ...
				                          <td colspan="2">		                            
				                            <select class="form-control" 
					                            required=""
					                            ng-model="vm.item.condition.code"
					                            ng-options="v.code as v.name for v in vm.params"
					                            ng-change="vm.formularChanged()">                                                 
						                     </select>
				                          </td>
				                          <td>
				                            <select class="form-control" 
					                            required=""
					                            ng-model="vm.item.condition.comparatorId"
					                            ng-options="v.id as v.name for v in vm.comparators"
					                            ng-change="vm.formularChanged()">                                                 
						                     </select>
				                          </td>		                          
				                          <td>
				                          	<input type="text" ng-model="vm.item.condition.value" class="form-control" placeholder="Saisir une valeur" required="" ng-change="vm.formularConditionChanged()"></input>
				                          </td>                         
				                        </tr>
				                        <tr>
				                          <td colspan="2">
				                          ...
				                          <td colspan="2">
				                          	<input type="number" step="any" ng-model="vm.item.condition.defaultValue" class="form-control" placeholder="Saisir une valeur par défaut" required="" ng-change="vm.formularConditionChanged()"></input>
				                          </td> 
				                          <td colspan="3">
				                          </td>                       
				                        </tr>
		                     ...
		                     <div class="form-group">
		                       <label class="form-label">
		                       		...
		                       		(<a href="javascript:void(0)" class="dropdown-toggle" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		                       			<i class="fa fa-plus"></i> Nouvelle expression
		                       		 </a>
		                       		 <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
									    <a class="dropdown-item" href="/collect/aggregated-model/formular/simple-expression/edit?model={{{{vm.item.rule_id}}}}&amp;formular={item/id}">Simple</a>
									    <a class="dropdown-item" href="/collect/aggregated-model/formular/case-expression/edit?model={{{{vm.item.rule_id}}}}&amp;formular={item/id}">Condition multiple</a>
									    <a class="dropdown-item" href="/collect/aggregated-model/formular/extended-to-parent-expression/edit?model={{{{vm.item.rule_id}}}}&amp;formular={item/id}">Etendre au parent</a>
									    <a class="dropdown-item" href="/collect/aggregated-model/formular/extended-to-child-expression/edit?model={{{{vm.item.rule_id}}}}&amp;formular={item/id}">Etendre à l'enfant</a>
									    <a class="dropdown-item" href="/collect/aggregated-model/formular/extended-to-model-expression/new?model={{{{vm.item.rule_id}}}}&amp;formular={item/id}" onclick="return confirm('Voulez-vous étendre le modèle ?');">Etendre au modèle</a>
									 </div>
		                       		)
		                       </label>
		                       <div class="table-responsive form-control">
				                    <table class="table card-table table-vcenter text-nowrap">
				                      <thead>
				                        <tr>
				                          <th ng-show="vm.ordering"></th>
				                          <th class="w-1">N°</th>
				                          <th>Libellé</th>
				                          <th>Résumé</th>
				                          <th>Type</th>
				                          <th colspan="2" class="text-right">
							                <a href="javascript:void(0)" role="button" class="btn btn-sm btn-primary mr-1" ng-click="vm.order()" ng-if="!vm.ordering">
							                  	<i class="fa fa-plus"></i> Ordonner
							                </a>
							                <a href="javascript:void(0)" role="button" class="btn btn-sm btn-primary mr-1" ng-click="vm.saveOrder()" ng-if="vm.ordering" ng-disabled="vm.inOrganizeMode">
							                  	<i class="fa fa-save"></i> Enregistrer ordre
							                </a>
				                          </th>
				                        </tr>
				                      </thead>
                ...
                <div class="card-footer text-right">
                 <div class="d-flex">                    
                    <a role="button" class="btn btn-primary" href="/collect/aggregated-model/edit?id={{{{vm.item.rule_id}}}}">
