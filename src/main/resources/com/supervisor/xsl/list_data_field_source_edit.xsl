...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:output method="html" cdata-section-elements="script style" include-content-type="no" doctype-system="about:legacy-compat" encoding="UTF-8" indent="yes"/>
  	...
  	<xsl:template match="page" mode="head">
	    <title>
	      <xsl:text>Supervisor - Minlessika - Configurer une source de données</xsl:text>
	    </title>
	</xsl:template>
           ...
           <form class="card" action="/collect/field/list/source/save" method="post">
               <xsl:if test="item and item/id">
		            <xsl:attribute name="action">
		              <xsl:text>/collect/field/list/source/save?id=</xsl:text><xsl:value-of select="item/id"/>
		            </xsl:attribute>
		        </xsl:if>
		        <div class="card-header">
			        ...
			        <div class="card-options">
	                  <xsl:choose>
	                  	<xsl:when test="table">
	                  		<a role="button" class="btn btn-primary" href="/collect/field/list/edit?id={field/id}&amp;model={model/id}&amp;table={table/id}&amp;table_model={table/model_id}">
			                  	<i class="fa fa-arrow-left"></i> Retour
			                </a>	                  		
	                  	</xsl:when>
	                  	<xsl:otherwise>
	                  		<a role="button" class="btn btn-primary" href="/collect/field/list/edit?id={field/id}&amp;model={model/id}">
			                  	<i class="fa fa-arrow-left"></i> Retour
			                </a>	                  		
	                  	</xsl:otherwise>
	                  </xsl:choose>
                 ...
                 <div class="row">
                   <div class="col-sm-12 col-md-12">
                     <div class="form-group">
                       ...
                       <select name="list_model_id" class="form-control" 
                       		required="" 
                       		ng-model="vm.modelId"
                   ...
                   <div class="col-sm-6 col-md-3">
                     <div class="form-group">
                       ...
                       <select name="field_to_display_id" class="form-control" 
                       		required="" 
                       		ng-model="vm.fieldToDisplayId">
                       		<option value="">-- None --</option>
