			...
			<form action="/activity/save" method="post" ng-controller="AppController as vm">
				<xsl:if test="item and item/id">
		            <xsl:attribute name="action">
		              <xsl:text>/activity/save?id=</xsl:text><xsl:value-of select="item/id"/>
		            </xsl:attribute>
		        </xsl:if>
				<div class="card">
	                <div class="card-header">	                  	                  
	                  	<xsl:if test="item and item/id">
				            <h3 class="card-title"><xsl:text>Propriétés de l'activité </xsl:text><i><xsl:value-of select="item/name"/></i></h3>
				        </xsl:if>
				        ...
				        <div class="card-options">
		                   <xsl:if test="item/id and item/is_template = 'true'">
				                <a role="button" class="btn btn-primary" href="/activity/template/change-designer/edit?activity={item/id}">
				                  	<i class="fa fa-plus"></i> Changer le concepteur
				                </a>
		                    </xsl:if>
		                </div>
	                </div>
				                        ...
				                        <tr>
				                          ...
				                          <th class="text-right">
				                            <xsl:if test="item/is_template = 'false'">
					                            <a href="javascript:void(0)" ng-click="vm.addActivityParam()" role="button" class="btn btn-sm btn-primary">
