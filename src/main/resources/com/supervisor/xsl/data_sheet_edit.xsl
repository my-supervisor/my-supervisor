...
<xsl:stylesheet
	...
	<xsl:template match="page" mode="body">
		<div class="col-12" ng-controller="AppCtrl as vml">
			<form class="card"
				action="/collect/sheet/save?model={data_sheet_model/id}"
				method="post">
				<xsl:if test="item and item/id">
					<xsl:attribute name="action">
		              <xsl:text>/collect/sheet/save?id=</xsl:text><xsl:value-of
						select="item/id" />
		            </xsl:attribute>
				</xsl:if>
				<div class="card-header">
					...
					<xsl:if test="not(item)">
						<h3 class="card-title">
							Nouvelle feuille -
							<i>
								<xsl:value-of select="data_sheet_model/name" />
							</i>
						</h3>
					</xsl:if>
					<div class="card-options">
						<a role="button" class="btn btn-primary"
							href="/collect/sheet/edit?model={data_sheet_model/id}">
							<i class="fa fa-plus" />
							Nouveau
						</a>
					</div>
		 	...
		 	<form ng-submit="vm.save()">
					 ...
					 <div class="row">
					 	<div class="col-md-12">
					 		<h5 class="pull-left">
			              	...
			              	<button type="button" class="btn btn-sm btn-primary pull-right" ng-click="vm.search()">
				...
				<div class="modal-footer">
	                <div class="dropdown" ng-if="vm.listModels.length > 1">
		                <a href="javascript:void(0)" role="button" class="dropdown-toggle btn btn-primary mr-1" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
	                ...
	                <a ng-if="vm.listModels.length == 1" href="/collect/sheet/edit?model={{{{vm.listModels[0].id}}}}" role="button" class="btn btn-primary mr-1" target="_blank">
	                  	<i class="fa fa-plus"></i> New
	                </a>	                
	...
	<xsl:template match="page" mode="customScript">
        ...
        <![CDATA[	           	
			...
			app.config(["$controllerProvider", "$provide", function($controllerProvider, $provide){
				...
				<xsl:for-each select="data_field_of_sheets/data_field_of_sheet">
					<xsl:if test="type_id = 'TABLE'">
           				<![CDATA[$templateCache.put(']]><xsl:value-of select="code"/><![CDATA[RowTemplate.html', angular.element("#]]><xsl:value-of select="code"/><![CDATA[RowTemplate")[0].innerHTML);]]>
           			</xsl:if>
				</xsl:for-each>
            		<![CDATA[
		    ...
		    app.controller("AppCtrl", ["$uibModal", "$rootScope", function($uibModal, $rootScope){
		        ...
		        var predefinedValues = []]> 
						   <xsl:for-each select="data_field_of_sheets/data_field_of_sheet">                             	                     
		                       	<xsl:if test="style_id = 'LIST' and can_be_updated = 'false'">
		                   ...
		                   <xsl:for-each select="rows/row/cell">                             	                     
		                       	<xsl:if test="style_id = 'LIST' and can_be_updated = 'false'">
		                       	<xsl:for-each select="predefined_value">
		                       	{	
		                       		'id': '<xsl:value-of select="id"/>', 
			...
			<table class="table card-table table-vcenter text-nowrap" style="min-width: 750px">
				<thead>
					<tr>
						...
						<th class="text-right">
							<a href="javascript:void(0)" ng-click="vm.addRow()" role="button" class="btn btn-sm btn-primary">
								<i class="fa fa-plus" />
								Nouveau
							</a>
						</th>
				...
				<tbody>
					<tr ng-repeat="row in vm.item.rows" ng-hide="row.state == 'removed'">
                        ...
                        <td cells="" url="'{$table/code}RowTemplate.html'" row="row"></td>
                        <td>
	...
	<xsl:template match="data_field_of_sheet" mode="number">
	    <xsl:variable name="code">
	    	<xsl:choose>
	    		<xsl:when test="table">
	    			<xsl:text>row_</xsl:text><xsl:value-of select="table_code"/><xsl:text>_</xsl:text><xsl:value-of select="code"/>
	    		</xsl:when>
	    		<xsl:otherwise>
				...
				<xsl:choose>
					<xsl:when test="can_be_updated = 'true'">
					    ...
					    <div class="input-group" ng-hide="vm.item{$code}.isReadonly">
						  ...
						  <div class="input-group-append">
						    <a ng-show="vm.item{$code}.id" class="btn btn-outline-primary" href="/collect/sheet/edit-by-ref?ref={{{{vm.item{$code}.id}}}}" target="_blank"><i class="fa fa-eye"></i></a>
					...
					<xsl:otherwise>
						<select class="form-control" 
							ng-required="vm.item{$code}.isMandatory"
							ng-model="vm.item{$code}.id" 
							ng-disabled="vm.item{$code}.isReadonly"
							ng-change="$parent.listValueChanged(vm.item{$code}, vm.item{$code}.id)">
							<xsl:for-each select="predefined_value">
								<option>
