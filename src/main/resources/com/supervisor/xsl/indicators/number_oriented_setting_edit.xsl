...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	<xsl:output method="html" cdata-section-elements="script style" include-content-type="no" doctype-system="about:legacy-compat" encoding="UTF-8" indent="yes"/>
  	...
  	<xsl:template match="page" mode="head">
	    <title>
	      <xsl:text>Supervisor - Minlessika - Configurer un chiffre orienté</xsl:text>
	    </title>
	</xsl:template>
	<xsl:template match="page" mode="body">
		<div class="col-12">
           <form class="card" action="/number-oriented-setting/save?shortname=number-oriented&amp;source={source}" method="post" ng-controller="AppController as vm">
               <xsl:if test="item and item/id">
		            <xsl:attribute name="action">
		              <xsl:text>/number-oriented-setting/save?shortname=number-oriented&amp;source=</xsl:text><xsl:value-of select="source"/><xsl:text>&amp;id=</xsl:text><xsl:value-of select="item/id"/>
		            </xsl:attribute>
		        </xsl:if>
		        ...
		        <input type="text" name="activity_id" hidden="hidden" value="{activity/id}"/>
                 <div class="row">
                   ...
                   <div class="col-sm-6 col-md-4">
                     <div class="form-group">
                       <label class="form-label">Libellé au singulier <span class="form-required">*</span></label>
                       <input name="single_label" type="text" class="form-control" placeholder="Saisir un nom au singulier" required="">
                       		<xsl:if test="item">
					            <xsl:attribute name="value">
					              <xsl:value-of select="item/single_label"/>
					            </xsl:attribute>
					        </xsl:if>
                       </input>
                     </div>
                   ...
                   <div class="col-sm-6 col-md-4">
                     <div class="form-group">
                       <label class="form-label">Libellé au pluriel <span class="form-required">*</span></label>
                       <input name="plural_label" type="text" class="form-control" placeholder="Saisir un nom au pluriel" required="">
                       		<xsl:if test="item">
					            <xsl:attribute name="value">
					              <xsl:value-of select="item/plural_label"/>
					            </xsl:attribute>
					        </xsl:if>
                       </input>
                     </div>
