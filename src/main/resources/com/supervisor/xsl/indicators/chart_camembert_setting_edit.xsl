           ...
           <form class="card" action="/chart-camembert-setting/save?shortname=chart-camembert&amp;source={source}" method="post" ng-controller="AppController as vm">
               <xsl:if test="item and item/id">
		            <xsl:attribute name="action">
		              <xsl:text>/chart-camembert-setting/save?shortname=chart-camembert&amp;source=</xsl:text><xsl:value-of select="source"/><xsl:text>&amp;id=</xsl:text><xsl:value-of select="item/id"/>
		            </xsl:attribute>
		        </xsl:if>
