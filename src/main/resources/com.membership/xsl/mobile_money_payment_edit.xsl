		...
		<div class="col-12">
           <form class="card" action="/payment-method/mobile-money/finalize" method="post" ng-controller="AppCtrl as vm">
		       <div class="card-header">
		       		<h3 class="card-title"><xsl:text>Mobile payment of payment request N° </xsl:text><i><xsl:value-of select="request/reference"/> (<xsl:value-of select="request/amount"/>)</i></h3>
		       </div>
               <div class="card-body">  
        ...
        <![CDATA[		
			...
			app.controller("AppCtrl", ["$scope", "$http", function ($scope, $http) {
				   ...
				   this.$onInit = function(){
				   	    ...
				   	    <xsl:choose>
				   	    	<xsl:when test="mobile_payment_receipt">
				   	    		vm.alreadySend = true;
