...
public final class PgSimpleOrdering implements Ordering {
	...
	@SafeVarargs
	public <T> PgSimpleOrdering(final String alias, final OrderDirection direction, Class<T> clazz, MethodRefWithoutArg<T>... methodRef) throws IOException {
	...
	@SafeVarargs
	public <T> PgSimpleOrdering(final OrderDirection direction, Class<T> clazz, MethodRefWithoutArg<T>... methodRef) throws IOException {
		this(StringUtils.EMPTY, direction, fields(clazz, methodRef));
	}
	
	@SafeVarargs
	private static <T> List<String> fields(Class<T> clazz, MethodRefWithoutArg<T>... methodRef) throws IOException{
		List<String> fields = new ArrayList<>();
		for (MethodRefWithoutArg<T> m : methodRef) {
